...
const filteredContactsFixture = contactsFixture.filter((contact) => contact.type === ContactType.Customer);
...
describe('mutations', () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', '/api/contacts/*', (req) =>
      req.reply(filteredContactsFixture.find(({ id }) => id === req.url.split('/').pop()) as IContact)
    );
  ...
  it('should delete a contact', () => {
    ...
    cy.intercept('/api/contacts', (req) => {
      if (interceptCount === 0) {
        ...
        req.reply(filteredContactsFixture.filter(({ id }) => id !== '89222b2d-8d06-41ff-82cf-c989dd90de24'));
      }
    });
